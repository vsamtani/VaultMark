...
// IIFE, acting as a wrapper on the presentation
const view = (() => {
...
function generatePassword(minimumEntropy = 70, blockSize = 6) {
  // Default pattern abcdef-234567-pqrtuv
  // Easy to type on a touchscreen keyboard
  // Easy to read out
  // No easily confused characters (001lsz)
  // Entropy, assuming you know the pattern: 71 bits,
  // (a lot better than correct-horse-battery-staple at 44 bits)

  let entropy_alpha = Math.log2(Math.pow(22, blockSize));
  let entropy_num = Math.log2(Math.pow(8, blockSize));
  let p = "";
  let pwEntropy = 0;
  let alpha = true;
